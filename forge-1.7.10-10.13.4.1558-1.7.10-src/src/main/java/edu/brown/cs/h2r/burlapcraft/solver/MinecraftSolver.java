...
//Terrence
import roboticsProject.ParticleFilter;
import roboticsProject.Particle;
/**
...
public class MinecraftSolver {
	 ...
	 * @param plannerToUse 0: BFS; 1: A*; 2: ParticleFilter
	...
	public static void plan(int plannerToUse, boolean closedLoop, boolean place){
		...
		if(plannerToUse == 0){
			...
			ParticleFilter filter = new ParticleFilter (domain,  BurlapCraft.currentDungeon, map);
			filter.printPos();
			filter.localize();
		}//---------------------------------------------------------------------------------------------------------------------
		else{
			throw new RuntimeException("Error: planner type is " + planner + "; use 0 for BFS, 1 for A* or 2 for Particle Filter");
		}//---------------------------------------------------------------------------------------------------------------------
//		planner.setTf(tf);
